  ...
  export default {
    ...
    computed: {
      ...mapGetters(['isRoadEditable','panel_time','link_id','link_list','isVillageEditable','village_id','village_list',
        'distance_type','global_v_type','global_village','global_area','grid_heatmap','person_trip_3','cur_menu','villageType',
      'v_update_time']),
    },
    props: {
      title:{
    ...
    methods: {
      ...mapMutations(['updateRoadEditable','updateDisplayMode','updateTravelMode','updateOnlyDistrict','updateDistrictMode',
        'updateMergeDisplay','updateLineDistrict','updateDistanceType','updateTravelAnaly','updateVillageList','updateVillageMethod',
        'updateGridHeatmap','updateOverAll3','updateKernelRadius']),
      setPanelState(){
        this.isPanelOpen=!this.isPanelOpen;
        this.$emit('TravelCharacteristicsPanel');
      },
      applyAll(){
      ...
      setGridHeatType(newVal){
        this.updateGridHeatmap({'personTrip-3': newVal})
      },
      setTravelCharaParam(menuId){
        if(menuId === "personTrip-3"){
          this.smallShow = this.person_trip_3.grid_heatmap;
          if (this.person_trip_3.villageType === 'link'){
            this.villMethod = '自定义';
          } else if (this.person_trip_3.villageType === 'taz') {
            this.villMethod = '全局';
          }
          if (this.person_trip_3.traver_analy === 0){
            this.curAnaly = '平均出行距离';
          } else if (this.person_trip_3.traver_analy === 1) {
            this.curAnaly = '平均出行时间';
          }

          this.updateGridHeatmap({'personTrip-3': this.smallShow});
          this.updateVillageMethod(this.villMethod);
          this.updateTravelAnaly(this.person_trip_3.traver_analy)

        }else if(menuId === "init"){
          this.smallShow = MYCONF.PERSONTRIP_3.grid_heatmap;
          this.villMethod = '自定义';
          this.curAnaly = '平均出行距离';

          this.updateGridHeatmap({'personTrip-3': MYCONF.PERSONTRIP_3.grid_heatmap});
          this.updateVillageMethod('自定义');
          this.updateTravelAnaly(MYCONF.PERSONTRIP_3.traver_analy)
        }
    ...
    watch:{
      curAnaly(newVal){
        ...
        if (newVal === '平均出行距离'){
          val = 0;
        } else if (newVal === '平均出行时间') {
          val = 1;
        }
        this.updateTravelAnaly(val)
      },
      village_list(newVal){
      ...
      switch1(newVal){
        this.updateOverAll3(newVal)
      },
      heatmap_radius(newVal){
        this.updateKernelRadius({'personTrip-3': newVal});
        MapCommonLayer.heatmapRadiusChange('personTrip-3', newVal);
      },
    }
  }
