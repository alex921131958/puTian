  ...
  <div class="box">
    ...
    <transition name="fade">
      <div v-if="isPanelOpen">
        ...
        <p class="list">
          <span>分析内容<span>  出行强度见个体出行OD分析模块</span></span>
          <i-select v-model="curAnaly" style="width:10em;">
        ...
        <VillageList :noDataVill="noDataVill" :villageTitle="villageTitle" :villageList="villageList" villageType1="panel"
                     :class="{hidden:((villMethod === '全局' && global_v_type === 0) || switch1 === false)}"/>
      </div>
    </transition>
  </div>
</template>
  ...
  export default {
    ...
    mounted() {
      ...
      eventBus.$on("setParamEvent",(type) => {    //参数的撤销和初始化操作
        ...
        if(type === 'clear'){                //取消
          ...
          this.setResidencePanelParam("init");
        }
      });
    ...
    computed: {
      ...mapGetters(['isRoadEditable','panel_time','link_id','link_list','isVillageEditable','village_id','village_list',
        'distance_type','global_v_type','global_village','global_area','grid_heatmap','cur_menu','person_trip_4','villageType',
      'v_update_time']),
    },
    props: {
      title:{
    ...
    methods: {
      ...mapMutations(['updateRoadEditable','updateDisplayMode','updateTravelMode','updateOnlyDistrict','updateDistrictMode',
        'updateMergeDisplay','updateLineDistrict','updateDistanceType','updateTravelAnaly','updateResidenceList','updateVillageList',
        'updateVillageMethod','updateGridHeatmap','updateOverAll4','updateKernelRadius']),
      setPanelState(){
        this.isPanelOpen=!this.isPanelOpen;
        this.$emit('ResidencePanel');
      },
      applyAll(){
      ...
      setGridHeatType(newVal){
        this.updateGridHeatmap({'personTrip-4': newVal})
      },
      setResidencePanelParam(menuId){
        if(menuId === "personTrip-4"){
          this.smallShow = this.person_trip_4.grid_heatmap;
          if (this.person_trip_4.villageType === 'link'){
            this.villMethod = '自定义';
          } else if (this.person_trip_4.villageType === 'taz') {
            this.villMethod = '全局';
          }
          if (this.person_trip_4.residence_list === 0){
            this.curAnaly = '职住比';
          } else if (this.person_trip_4.residence_list === 1) {
            this.curAnaly = '居住人口';
          }else if (this.person_trip_4.residence_list === 2) {
            this.curAnaly = '就职人口';
          }
          this.updateGridHeatmap({'personTrip-4': this.smallShow});
          this.updateVillageMethod(this.villMethod);
          this.updateResidenceList(this.person_trip_4.residence_list)

        }else if(menuId === "init"){
          this.smallShow = MYCONF.PERSONTRIP_4.grid_heatmap;
          this.villMethod = '自定义';
          this.curAnaly = '居住人口';

          this.updateGridHeatmap({'personTrip-4': MYCONF.PERSONTRIP_4.grid_heatmap});
          this.updateVillageMethod('自定义');
          this.updateResidenceList(MYCONF.PERSONTRIP_4.residence_list)
        }
      },
    },
    watch:{
      ...
      switch1(newVal){
        this.updateOverAll4(newVal)
      },
      heatmap_radius(newVal){
        this.updateKernelRadius({'personTrip-4': newVal});
        MapCommonLayer.heatmapRadiusChange('personTrip-4', newVal);
      },
    }
  }
