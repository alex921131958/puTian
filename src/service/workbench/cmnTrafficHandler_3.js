...
export default class cmnTrafficHandler_3 {
  constructor(_view) {
    this._view = _view;
  }
  ...
  getPanelParams(){
    ...
    this._view.$http.get(url).then((res) => {
      ...
      if(res.result.length === 0){
        ...
        paramList.map((item) => {
          let key = item.key;
          tempObj[key] = key==='links'||key==='linktype'||key==='roadtype'?item.value:parseInt(item.value);
        });
        this._view.updateCmnTraffic3(tempObj);
        let panelVType = tempObj['linktype']==='taz'?0:tempObj['linktype']==='vil'?1:2;
        this._view.updatePanelVType(panelVType);
        if(this._view.cur_menu.id === "cmnTraffic-3"){
